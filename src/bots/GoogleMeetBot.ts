        ...
        await retryActionWithWait(
          ...
          async () => {
            ...
            const possibleTexts = [
              ...
              'Jetzt teilnehmen',
              'Trotzdem teilnehmen',
              // Spanish (tercerpiso fork) — robustness if the UI isn't forced to English
              'Unirse ahora',
              'Pedir unirse',
    ...
    await this.page.evaluate(
      ...
      { teamId:string, userId: string, duration: number, inactivityLimit: number, loneParticipantExitDelayMs: number, slightlySecretId: string, activateInactivityDetectionAfter: string, activateInactivityDetectionAfterMinutes: number, mimeTypes: string[], audioOnly: boolean, captureDiarization: boolean, diarizationPollMs: number }) => {
        ...
        async function startRecording() {
          ...
          // Audio-only (tercerpiso fork): record just the audio track — the
          // video is useless for transcription and ~10-15x heavier. Keeps the
          // full `stream` for silence/presence detection; only the recorder is
          // narrowed. Falls back to the full stream if opus audio isn't supported.
